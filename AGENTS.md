...
- **Structure**: `index.html` is the whole page (semantic sections: `#about`, `#career`, `#contacts`, `#tmi`). Styling is in `style.css` (CSS variables + responsive layout). Images/icons live in `files/`. `CNAME` sets the custom domain for GitHub Pages.
- **Third-party JS/CSS** (AOS scroll animations + Google Fonts Noto Sans KR) is loaded from CDNs at runtime. Scroll progress bar and the days counter are vanilla JS in `index.html` — no jQuery.
